...
const TopNavigation: FunctionComponent<TopNavigationType> = ({
  ...
  const renderNavLinks = () => (
    <>
      <div
        className="[text-decoration:none] h-6 w-[76px] relative leading-[24px] font-medium text-[inherit] flex items-center shrink-0 cursor-pointer"
        style={featuresStyle}
  ...
  const renderMobileNavLinks = () => (
    <nav className="flex flex-col items-center relative -top-10 mq750:-top-20 space-y-8 w-full max-w-md">
      <div
        className="text-lg font-semibold text-gray-800 hover:text-color no-underline transition-colors duration-300 relative group cursor-pointer"
        style={featuresStyle}
        onClick={handleFeatureClick}
      >
        Features
        <span className="absolute -bottom-2 left-0 w-0 h-0.5 bg-color group-hover:w-full transition-all duration-300" />
      </div>

      <RouterLink
        className="text-lg font-semibold text-gray-800 hover:text-color no-underline transition-colors duration-300 relative group"
        style={pricingStyle}
        to="/pricing"
      >
        Pricing
  ...
  return (
    <header
      className={`w-full max-w-full px-6 flex flex-row items-center justify-center mq1125:px-6 py-[25px] box-border ${className}`}
    >
      <div className="w-full max-w-[1440px] flex flex-row items-center justify-between">
        ...
        {isMobileMenuOpen && (
          <div className="fixed inset-0 z-50 flex items-stretch mq1125:flex hidden">
            <div
              onClick={toggleMobileMenu}
              className="bg-black/50 flex-1 cursor-pointer"
            />

            <div className="w-1/2 bg-white relative flex flex-col items-center justify-center p-8 overflow-hidden">
              <img
              ...
              <div
                onClick={toggleMobileMenu}
                className="absolute top-3 right-7 cursor-pointer duration-300"
              >
